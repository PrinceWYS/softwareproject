...
export default function Calindar () {
    ...
    useEffect(() => {
        ...
        const path = "http://localhost:3000/patient/myappointment/";
        ...
        <div width="100%">
        	<div width="100%">
        		<div className="tab">
                    ...
                    <thead>
                        <tr>
                            <td align="center" colSpan="8" className="textta"><strong>个人预约</strong></td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                                    ...
                                    return (
                                        <td key={index}>
                                            {tasks && tasks.map(task => {
                                                ...
                                                console.log(date >= today)
                                                console.log(date.getDay() == index)
                                                console.log(task.cal_num == 10151200)
                                                console.log("---")
                                                if (date >= today && date.getDay() == index && task.cal_num == 10151200) {
                                                    return (
                                                        <div key={index}>
                                                            <p>{task.docter_name}</p>
                                                            <p>{task.doctor_title}</p>
                                                        </div>
                                                    )
                                                }
                                            }
...
function LoadCalindar(path, setTasks) {
    Axios.get(path, {params: {patient_ID: localStorage.getItem('token')}})
        .then(res => {
            console.log(res.data.res);
            setTasks(res.data.res);
            // res.data should contain:
            // {cal_day -> cal_day, cal_num -> cal_num, doctor_name, description}
        })
}
